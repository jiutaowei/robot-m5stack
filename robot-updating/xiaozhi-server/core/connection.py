...
class ConnectionHandler:
    ...
    async def _route_message(self, message):
        ...
        if isinstance(message, str):
            ...
            # 服务端 AEC（v2 协议）：客户端上传的音频帧带 16 字节头
            # （BinaryProtocol2：version/type/reserved/timestamp/payload_size），
            # 需要剥头、取 timestamp 做回声消除后再解码。
            if self.client_aec and len(message) >= 16:
                handled = await self._process_websocket_v2_audio_message(message)
                if handled:
                    return
    ...
    async def _process_websocket_v2_audio_message(self, message):
        """
        处理服务端AEC（v2协议）客户端上传的音频消息。

        固件 BinaryProtocol2 头（16字节，大端）：
            [0:2]   version
            [2:4]   type (0: OPUS, 1: JSON)
            [4:8]   reserved
            [8:12]  timestamp（播放TTS时记录的，用于对齐参考音频）
            [12:16] payload_size
            [16:]   payload（Opus）

        与 MQTT 网关头共用相同的 timestamp 偏移（[8:12]）和 payload 偏移（[16:]），
        因此解析逻辑一致，区别只在是否校验版本号。
        ...
        try:
            ...
            if version != 2:
                # 不是 v2 帧，交给上层 v1 路径处理
                return False

            timestamp = int.from_bytes(message[8:12], "big")
            payload_size = int.from_bytes(message[12:16], "big")
            if payload_size > len(message) - 16:
                self.logger.bind(tag=TAG).warning(
                    f"[AEC] v2帧payload长度异常: header={payload_size} actual={len(message)-16}"
                )
                return False

            audio_data = message[16 : 16 + payload_size]
            pcm_frame = self._decode_opus_packet(audio_data)
            if not pcm_frame:
                return True

            # AEC处理：用刚播出的TTS音频做参考消除扬声器回声
            if timestamp > 0 and self.client_aec:
                pcm_frame = self._apply_aec(timestamp, pcm_frame)
            else:
                self.logger.bind(tag=TAG).debug(
                    f"[AEC] v2帧未做AEC: ts={timestamp} aec={self.client_aec} cache={len(getattr(self, 'aec_audio_cache', {}))}"
                )

            self.asr_audio_queue.put(pcm_frame)
            return True
        except Exception as e:
            self.logger.bind(tag=TAG).error(f"解析v2音频包失败: {e}")
    ...
    async def _initialize_private_config_async(self):
        ...
        elif "TTS" in self.config and self.config.get("selected_module", {}).get("TTS"):
            # 差异化配置未提供 TTS 时，fallback 使用本地配置（data/.config.yaml）
            # 保证 TTS 一定初始化（否则 AI 只显示文字不出声）
            init_tts = True
        ...
        try:
            modules = await self.loop.run_in_executor(
                ...
                initialize_modules,
                self.logger,
                self.config,  # 完整配置（含本地 + 差异化），保证 TTS 等模块可初始化
                init_vad,
                init_asr,
                init_llm,
                init_tts,
