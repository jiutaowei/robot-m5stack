                                         ...
                                         const lv_font_t* font)
{
    ...
    _container->addFlag(LV_OBJ_FLAG_OVERFLOW_VISIBLE);  // 允许三角超出容器显示（不被裁剪）
    ...
    // 三角移到最上层，避免被气泡/其他元素遮挡
    _arrow->moveForeground();
...
void DefaultSpeechBubble::setSpeech(std::string_view text)
{
    ...
    // 米宝回答：白色气泡+黑色文字，三角固定在最初位置（椭圆左上方的 x=40）
    _bubble->setBgOpa(LV_OPA_COVER);
    _bubble->setBgColor(lv_color_white());
    _text->setTextColor(lv_color_black());
    _arrow->setImageRecolorOpa(LV_OPA_COVER);
    _arrow->setImageRecolor(lv_color_white());
...
void DefaultSpeechBubble::setSpeechUser(std::string_view text)
{
    if (text.empty()) {
        clearSpeech();
        return;
    ...
    // 我说话：浅蓝气泡+深色文字，三角与椭圆同色（浅蓝）
    lv_color_t user_color = lv_color_hex(0x9ECBFF);
    _bubble->setBgOpa(LV_OPA_COVER);
    _bubble->setBgColor(user_color);
    _text->setTextColor(lv_color_hex(0x1F3A5F));
    _arrow->setImageRecolorOpa(LV_OPA_COVER);
    _arrow->setImageRecolor(user_color);
    _arrow->setRotation(0);
    _arrow->setPos(_arrow_offset.x, _arrow_offset.y);
    _arrow->moveForeground();
    _container->moveForeground();
...
void DefaultSpeechBubble::showText(std::string_view text)
{
    _text->setText(text);
