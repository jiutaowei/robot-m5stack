...
static constexpr std::string_view kShanghaiTimezone = "CST-8";
...
void Hal::rtc_init()
{
    ...
    // The display is intended for Shanghai.  Older builds stored GMT0 by
    // default, which made correctly synchronized NTP time appear eight hours
    // behind.  Migrate only that legacy default; an explicit user selection in
    // Settings remains respected.
    std::string tz = getTimezone();
    if (tz.empty() || tz == "GMT0") {
        tz = kShanghaiTimezone;
        setTimezone(tz);
    }
...
std::string Hal::getTimezone()
{
    ...
    return settings.GetString("tz", kShanghaiTimezone.data());
}
