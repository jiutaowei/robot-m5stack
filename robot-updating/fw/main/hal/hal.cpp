...
static void _stackchan_update_task(void* param)
{
    ...
    while (1) {
        ...
        // 注意：不能在 xiaozhi 非 idle 时延迟 100ms——手势检测（HomeGesture/
        // StatuBarGesture）靠每帧采样触摸状态，采样间隔太长会漏掉快速上滑/下滑，
        // 导致 AI 对话界面无法呼出 home 按钮和顶部状态栏。保持 20ms 循环。
        ...
        if (!is_setup_done) {
            ...
            is_setup_done = true;
            mclog::tagInfo(_tag, "xiaozhi home/status bar created");
        }
...
void Hal::startXiaozhi()
{
    ...
    // 创建任务启动 xiaozhi，避免阻塞主循环
    xTaskCreate([](void*) {
        hal_bridge::start_xiaozhi_app();
        vTaskDelete(NULL);
    }, "xiaozhi_start", 8192, nullptr, 2, nullptr);
}
