...
bool ownerMatches(const std::string& storedOwner, std::string_view owner)
{
    return storedOwner.size() == owner.size() && storedOwner.compare(0, storedOwner.size(), owner.data(), owner.size()) == 0;
}
...
bool Hal::acquireMicrophone(std::string_view owner)
{
    if (owner.empty()) {
        return false;
    }
    if (!_microphone_owner.empty() && !ownerMatches(_microphone_owner, owner)) {
        mclog::tagWarn(_tag, "microphone already owned by %s", _microphone_owner.c_str());
        return false;
    }
    if (ownerMatches(_microphone_owner, owner)) {
        return true;
    }
    ...
    auto audio_codec = board.GetAudioCodec();
    if (!audio_codec) {
        mclog::tagError(_tag, "audio codec unavailable for microphone owner %.*s",
                        static_cast<int>(owner.size()), owner.data());
        return false;
    }

    _microphone_owner                   = std::string(owner);
    _meeting_input_was_enabled          = audio_codec->input_enabled();
    _meeting_output_was_enabled         = audio_codec->output_enabled();
    _meeting_speaker_volume             = getSpeakerVolume();
    _meeting_servo_power_was_enabled    = _servo_power_enabled;
    _meeting_audio_saved                = true;

    if (_meeting_output_was_enabled) {
        audio_codec->EnableOutput(false);
    }
    setSpeakerVolume(0);
...
void Hal::releaseMicrophone(std::string_view owner)
{
    if (!ownerMatches(_microphone_owner, owner)) {
        return;
    }
    ...
    auto& board      = Board::GetInstance();
    auto audio_codec = board.GetAudioCodec();
    if (audio_codec && _meeting_audio_saved) {
        audio_codec->EnableInput(_meeting_input_was_enabled);
        audio_codec->EnableOutput(_meeting_output_was_enabled);
    }
    setSpeakerVolume(_meeting_speaker_volume);
    setServoPowerEnabled(_meeting_servo_power_was_enabled);
    _meeting_audio_saved = false;
    _microphone_owner.clear();
}

bool Hal::isMicrophoneOwnedBy(std::string_view owner)
{
    return !_microphone_owner.empty() && ownerMatches(_microphone_owner, owner);
}
