...
/* SD card pins.
 * M5Stack CoreS3 uses the display SPI bus for the TF/SD socket:
 *   CS=GPIO4, SCK=GPIO36, MISO=GPIO35, MOSI=GPIO37.
 * This matches the official M5Unified SD example. GPIO35/36/37 are shared
 * with the LCD, so stackchan.cc switches all shared GPIO-matrix routes during
 * SD access and restores them afterwards.
 */
#define SDCARD_ENABLED     1
#define SDCARD_MOUNT_POINT "/sdcard"
// Keep the controllers separate. The physical pins are shared and are
// explicitly routed to one controller at a time by stackchan.cc.
