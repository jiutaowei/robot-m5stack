...
bool StackChanCamera::Capture()
{
    ...
    // Play shutter sfx（录像等逐帧抓取场景需先 SetPlayShutterSound(false)，
    // 否则 mooncake 模式下 xiaozhi 音频服务未初始化会崩溃）
    if (play_shutter_sound_) {
...
bool StackChanCamera::GrabFrame()
{
    if (encoder_thread_.joinable()) {
        encoder_thread_.join();
    }
    ...
    // 保存帧副本到 PSRAM（与 Capture 相同的复制逻辑）
    if (frame_.data) {
        heap_caps_free(frame_.data);
        frame_.data   = nullptr;
        frame_.format = 0;
    ...
    switch (sensor_format_) {
        ...
        case V4L2_PIX_FMT_JPEG:
#endif  // CONFIG_XIAOZHI_CAMERA_ALLOW_JPEG_INPUT
#if defined(CONFIG_XIAOZHI_ENABLE_CAMERA_ENDIANNESS_SWAP)
            {
                auto src16   = (uint16_t*)mmap_buffers_[buf.index].start;
                auto dst16   = (uint16_t*)frame_.data;
                size_t count = (size_t)mmap_buffers_[buf.index].length / 2;
...
bool StackChanCamera::PauseStream()
{
    if (!streaming_on_ || video_fd_ < 0) {
        return false;
    }
...
bool StackChanCamera::ResumeStream()
{
    if (streaming_on_ || video_fd_ < 0) {
        return false;
    }
