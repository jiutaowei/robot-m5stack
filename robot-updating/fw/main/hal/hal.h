...
struct EspNowRemoteCommandEvent {
    EspNowRemoteCommand command = EspNowRemoteCommand::None;
    uint8_t sequence            = 0;
};
...
public:
    ...
    void requestXiaozhiExit()
    {
        _xiaozhi_exit_requested = true;
    }
    bool isXiaozhiExitRequested()
    {
        return _xiaozhi_exit_requested;
    }
    void resetXiaozhiExitRequest()
    {
        _xiaozhi_exit_requested = false;
    }
    void startXiaozhi();
    ...
    bool pollEspNowRemoteCommand(EspNowRemoteCommandEvent& event);
    ...
    bool acquireMicrophone(std::string_view owner);
    void releaseMicrophone(std::string_view owner);
    bool isMicrophoneOwnedBy(std::string_view owner);
...
private:
    ...
    bool _xiaozhi_exit_requested = false;
    bool _servo_power_enabled = true;
    std::string _microphone_owner;
    bool _meeting_audio_saved = false;
    bool _meeting_input_was_enabled = false;
    bool _meeting_output_was_enabled = false;
    bool _meeting_servo_power_was_enabled = true;
    uint8_t _meeting_speaker_volume = 0;
...
/**
 * @brief 硅基一号温室执行器控制（MCP 工具与物联网触屏 UI 共用）
 *
 * 同步 HTTP POST 到 mibao_config.iot_url + "/actuator_control"，5s 超时，
 * 返回 JSON 字符串（含 success 字段）。actuator: fan/pump/light/heat，
 * action: on/off，duration_seconds 0=持续运行。
 */
std::string controlGreenhouseActuator(const std::string& actuator, const std::string& action,
                                      int durationSeconds);
