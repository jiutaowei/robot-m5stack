...
void Toast::init(lv_obj_t* parent)
{
    ...
    // toast 文案可能含中文：以 montserrat_20 为基底、中文回退到米宝字体。
    // 基底字体是 flash 只读 const，不能直接写 fallback，用 RAM 副本挂接。
    static lv_font_t s_toast_font_rt;
    static bool toast_font_hooked = false;
    if (!toast_font_hooked) {
        std::memcpy(&s_toast_font_rt, &lv_font_montserrat_20, sizeof(lv_font_t));
        s_toast_font_rt.fallback = &mibao_zh_font;
        toast_font_hooked        = true;
    }
    _msg_label->setTextFont(&s_toast_font_rt);
