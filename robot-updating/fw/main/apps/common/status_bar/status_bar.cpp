...
public:
    ...
    void update()
    {
        ...
        // 否则仅在抬起瞬间 mark，App 的 onRunning 同帧更早检查时抑制不生效，
        // 顶部下滑的状态栏手势会误触到顶部按钮/区域。
...
public:
    StatusBarView(lv_obj_t* parent, uint32_t colorSecondary, uint32_t colorPrimary)
    {
        ...
        // This is an always-on system status bar.  Do not attach a click handler:
        // touching it must not hide the Wi-Fi, battery, or clock indicators.
    ...
    void show()
    {
        ...
        _is_hidden  = false;
        // 召唤时立即刷新一次（时间/电量/WiFi），避免显示隐藏期间缓存的旧状态
        _last_update_tick = 0;
        for (auto& widget : _widgets) {
            widget->update();
        }
...
public:
    void init(lv_obj_t* parent, uint32_t colorSecondary, uint32_t colorPrimary)
    {
        ...
        // 默认隐藏，从顶部下滑显示（对齐官方 StackChan 交互）
