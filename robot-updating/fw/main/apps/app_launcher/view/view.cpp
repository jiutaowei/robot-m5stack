...
// 米宝精简中文字体：launcher 标签优先使用，西文回退到 MontserratSemiBold26。
...
public:
    ...
    // The persistent 28px system status bar occupies the top of the screen.
    // Keep the launcher app name below it instead of drawing underneath it.
    const int visible_pos_y   = -76;
    ...
    void init(const std::vector<std::string>& iconLabelTexts, int iconGap, lv_obj_t* parent)
    {
        ...
        // 一次性挂接西文 fallback，保证中文用米宝字体、西文保持原观感。
        // 注意：mibao_zh_font 是 const 字体，位于 flash 只读段，直接写其
        // fallback 字段会触发 StoreProhibited panic 导致重启，
        // 必须复制到 RAM 副本后再挂接。
        static lv_font_t s_label_font_rt;
        static bool fallback_hooked = false;
        if (!fallback_hooked) {
            std::memcpy(&s_label_font_rt, &mibao_zh_font, sizeof(lv_font_t));
            s_label_font_rt.fallback = &MontserratSemiBold26;
            fallback_hooked          = true;
        }
        _label = std::make_unique<Label>(parent);
        _label->setTextColor(lv_color_hex(0x000000));
        _label->setTextFont(&s_label_font_rt);
...
void LauncherView::init(std::vector<mooncake::AppProps_t> appPorps)
{
    ...
    // 米宝主题色 #2DBE8D（DEVELOPMENT_PLAN §1），滚动时由各 app userData 主题色接管
    _panel->setBgColor(lv_color_hex(0x2DBE8D));
    _panel->addFlag(LV_OBJ_FLAG_SCROLL_ONE);
    _panel->setScrollDir(LV_DIR_HOR);
    lv_obj_set_scroll_snap_x(_panel->get(), LV_SCROLL_SNAP_CENTER);
    lv_obj_set_scrollbar_mode(_panel->get(), LV_SCROLLBAR_MODE_OFF);
    lv_obj_set_style_anim_duration(_panel->get(), 80, 0);
    ...
    _display_app_ids.clear();
    for (int loop = 0; loop < _loop_copies; loop++) {
        for (const auto& props : appPorps) {
            // 记录真实 app 的 appID（第一轮循环），供 warm-boot 自动打开
            if (loop == 0) {
                _display_app_ids.push_back(props.appID);
            }
            // Icon panel
            ...
            if (props.info.icon != nullptr) {
                ...
                // 米宝图标为 150×150，居中后顶部会顶到动态文字标签，缩小到约 111px 避免重叠（#3）
                _icon_images.back()->setScale(190);
            }
    ...
    _lr_indicator_panels.back()->onClick().connect([scroll_to_nearby_icon]() {
        if (view::swipe_recently_detected(400)) {
    ...
    if (GetHAL().getWarmRebootTarget() >= 0) {
        ...
        // 语音指令（如"打开会议录制"）请求的 warm boot：自动打开目标 app，
        // 无需用户点击。状态机初始化完成后 handle_state_normal 会触发 onAppClicked。
        if (app_index < static_cast<int>(_display_app_ids.size())) {
            _clicked_app_id = _display_app_ids[app_index];
            mclog::tagInfo(_tag, "warm boot auto open app id: {}", _clicked_app_id);
        }
    }
