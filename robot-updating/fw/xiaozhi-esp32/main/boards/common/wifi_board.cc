...
void WifiBoard::TryWifiConnect() {
    ...
    if (have_ssid) {
        // 米宝场景：xiaozhi 在 mooncake 阶段 WiFi 已连上（用户先进桌面再点 AI对话），
        // WifiManager::StartStation() 看到 Station already active 会直接 return，
        // 不再触发 WifiEvent::Connected，导致 xiaozhi 的 ActivationTask 永远不启动。
        // 修复：如果 WiFi 已连，主动注入 Connected 事件，让 xiaozhi 继续走 OTA 检查流程。
        auto& wifi_manager = WifiManager::GetInstance();
        if (wifi_manager.IsConnected()) {
            ESP_LOGI(TAG, "WiFi already connected, inject Connected event to trigger activation");
        ...
        // No SSID configured.  Boards may gate auto provisioning so the
        // config hotspot only starts via an explicit user action.
        if (!AllowAutoWifiConfigMode()) {
            ESP_LOGI(TAG, "No SSID configured, auto config mode disabled by board policy");
            return;
        }
...
void WifiBoard::OnNetworkEvent(NetworkEvent event, const std::string& data) {
    switch (event) {
...
#endif
            // 记录本轮是否配过网（在 in_config_mode_ 置 false 前读取）
            const bool was_provisioned = provisioned_this_session_;
            in_config_mode_ = false;
            provisioned_this_session_ = false;
            ESP_LOGI(TAG, "Connected to WiFi: %s", data.c_str());

            // 配网成功（本轮从 SoftAP 配网提交凭证后连上）→ 主动重启回 launcher。
            // 这是米宝产品要求：配网完成后回到米宝 launcher 界面，而非停留在 xiaozhi。
            // 重启后 SSID 已保存在 NVS，launcher 检测到 SSID 直接正常显示。
            if (was_provisioned) {
                ESP_LOGI(TAG, "Provisioning success, rebooting to launcher in 2s");
                xTaskCreate([](void*) {
                    vTaskDelay(pdMS_TO_TICKS(2000));
                    esp_restart();
                    vTaskDelete(NULL);
                }, "prov_restart", 4096, nullptr, 5, nullptr);
            }
            break;
        }
        ...
        case NetworkEvent::WifiConfigModeExit:
            ...
            in_config_mode_ = false;
            // 配网模式退出（用户提交凭证后）→ 标记本轮配过网，
            // 连接成功后自动重启回 launcher（见 Connected 分支）。
            provisioned_this_session_ = true;
...
void WifiBoard::OnWifiConnectTimeout(void* arg) {
    ...
    if (!board->AllowAutoWifiConfigMode()) {
        // Gate: stay offline instead of starting the provisioning hotspot.
        // Provisioning can still be entered manually (Hotspot Setup menu).
        ESP_LOGW(TAG, "WiFi connection timeout, auto config mode disabled by board policy");
