...
void AudioService::WaitForPlaybackQueueEmpty() {
    std::unique_lock<std::mutex> lock(audio_queue_mutex_);
    // 带超时等待：队列清空是"防止上一句音频截断"的优化项，不能因此卡死主流程。
    // 实测（Qwen3 ASR 会话）队列残留时无超时等待会永远阻塞，导致后续
    // EnableVoiceProcessing / EnableWakeWordDetection 不执行，麦克风不采集，
    // 表现为"说话/喊话都没反应"。
    if (!audio_queue_cv_.wait_for(lock, std::chrono::milliseconds(1000), [this]() {
        ...
        ESP_LOGW(TAG, "WaitForPlaybackQueueEmpty timeout, decode=%d playback=%d",
                 (int)audio_decode_queue_.size(), (int)audio_playback_queue_.size());
    }
}
