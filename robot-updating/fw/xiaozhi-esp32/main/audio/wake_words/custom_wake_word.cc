...
void CustomWakeWord::ParseWakenetModelConfig() {
    ...
    // 补充唤醒词：你好米宝 / 米宝同学（与待机唤醒一致；index.json 只配了米宝米宝）
    AddWakeWordCommand("ni hao mi bao", "你好米宝");
    AddWakeWordCommand("mi bao tong xue", "米宝同学");
}
...
void CustomWakeWord::Feed(const std::vector<int16_t>& data) {
    ...
    // 每次 Feed 只处理一个 chunk：避免音频堆积时 while 循环连续跑
    // multinet_->detect()（单次最长 3s 超时）占满 audio_input 任务，
    // 导致 IDLE 饿死触发 task_wdt、唤醒检测卡死。
    if (input_buffer_.size() >= chunksize) {
