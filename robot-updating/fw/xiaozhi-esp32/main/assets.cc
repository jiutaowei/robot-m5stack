...
bool Assets::LvglStrategy::InitializePartition(Assets* assets) {
    ...
    // The partition can reserve more flash than the current asset image uses (for example when
    // a Multinet model is added).  Map only the encoded image, otherwise an enlarged partition
    // can exhaust the MMU mapping space and make every application icon unavailable at boot.
    uint32_t header[3] = {};
    ...
    if (stored_len > assets->partition_->size - 12) {
        ESP_LOGE(TAG, "The stored_len (0x%lx) is greater than the assets partition (0x%lx) - 12", stored_len,
                 assets->partition_->size);
        return false;
    }

    const size_t mapped_size = 12 + stored_len;
    ...
    ESP_LOGI(TAG, "The assets image size is %d KB (partition: %ld KB)", int(mapped_size / 1024),
             assets->partition_->size / 1024);
    if (storage_size < mapped_size) {
        ESP_LOGE(TAG, "The free size %ld KB is less than the assets image required %d KB", storage_size / 1024,
                 int(mapped_size / 1024));
        return false;
    }

    err = esp_partition_mmap(assets->partition_, 0, mapped_size, ESP_PARTITION_MMAP_DATA, (const void**)&mmap_root_,
                             &mmap_handle_);
    if (err != ESP_OK) {
