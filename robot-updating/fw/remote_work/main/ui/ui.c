...
void switch_screen(int screen_id)
{
    ...
    } else if (screen_id == MODE_MEETING) {
    meeting_create:
        if (meeting_screen == NULL) {
            create_meeting_screen();
            ESP_LOGI("UI", "Meeting screen created");
        }
        if (meeting_screen != NULL && lv_obj_is_valid(meeting_screen)) {
            ESP_LOGI("UI", "Meeting screen loaded");
            lv_scr_load_anim(meeting_screen, LV_SCR_LOAD_ANIM_MOVE_LEFT, 200, 0, false);
        } else {
            ESP_LOGE("UI", "Meeting screen is NULL or invalid!");
            ui_meeting_screen_destroy();
            goto meeting_create;
        }
    } else {
