...
joystick_data_t joystick_init()
{
    ...
    tmp.remote_command = REMOTE_COMMAND_NONE;
    tmp.remote_command_sequence = 0;
    tmp.remote_command_repeats = 0;
    return tmp;
}
...
{
    joystick_data_t *joystick_data = (joystick_data_t *)pvParam;
    // Original packet: target id, yaw, pitch, speed, and BtnB status.
    uint8_t pkt[8] = {};
...
/** Send meeting commands separately from the original movement modes. */
void handle_meeting_screen(void *pvParam)
{
    joystick_data_t *joystick_data = (joystick_data_t *)pvParam;
    uint8_t pkt[9] = {};
    int16_t zero = 0;
    int16_t speed = 600;
    pkt[1] = pkt[2] = 0;
    memcpy(&pkt[1], &zero, sizeof(zero));
    memcpy(&pkt[3], &zero, sizeof(zero));
    memcpy(&pkt[5], &speed, sizeof(speed));

    while (1) {
        if (joystick_data->screen_mode == MODE_MEETING && joystick_data->remote_command_repeats > 0) {
            pkt[0] = (uint8_t)joystick_data->id;
            pkt[7] = joystick_data->remote_command;
            pkt[8] = joystick_data->remote_command_sequence;
            espnow_send_data(pkt, sizeof(pkt));
            joystick_data->remote_command_repeats--;
            vTaskDelay(30 / portTICK_PERIOD_MS);
        } else {
