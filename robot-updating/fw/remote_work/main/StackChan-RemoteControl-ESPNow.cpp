 ...
 * 1. Press BtnA to cycle setup, running, IMU, and meeting modes.
 * 2. Press BtnB to switch espnow-channel or id on setup_mode;
 * 3. Press BtnB to toggle the original button state in running/IMU modes.
 * 4. Short/long press BtnB controls the meeting in meeting mode.
 */
static void queue_remote_command(uint8_t command)
{
    joystick_data.remote_command = command;
    joystick_data.remote_command_sequence++;
    joystick_data.remote_command_repeats = 5;
    update_meeting_screen(command);
}

void handle_button_press()
{
    static uint8_t screen_mode = MODE_SETUP;
    static bool btn_b_tracking = false;
    static bool btn_b_long_sent = false;
    constexpr uint32_t kBtnBLongPressMs = 1000;
    ...
    if (M5.BtnA.wasPressed()) {
        ...
        if (screen_mode == MODE_SETUP) {
            ...
            // in IMU mode, press A to enter meeting mode
            joystick_data.screen_mode = MODE_IMU;
            switch_screen(joystick_data.screen_mode);
        } else if (screen_mode == MODE_MEETING) {
            wifi_espnow_reinit(joystick_data.channel);
            joystick_data.screen_mode = MODE_MEETING;
            switch_screen(joystick_data.screen_mode);
        }
    }
    ...
    if ((joystick_data.screen_mode == MODE_RUNNING) || (joystick_data.screen_mode == MODE_IMU)) {
        if (M5.BtnB.wasPressed()) {
    ...
    if (joystick_data.screen_mode == MODE_MEETING) {
        if (M5.BtnB.wasPressed()) {
            btn_b_tracking = true;
            btn_b_long_sent = false;
        }

        if (btn_b_tracking && !btn_b_long_sent && M5.BtnB.pressedFor(kBtnBLongPressMs)) {
            queue_remote_command(REMOTE_COMMAND_END);
            btn_b_long_sent = true;
        }

        if (btn_b_tracking && M5.BtnB.wasReleased()) {
            if (!btn_b_long_sent) {
                queue_remote_command(REMOTE_COMMAND_PRIMARY);
            }
            btn_b_tracking = false;
        }
    }
}
...
void app_main(void)
{
    ...
    xTaskCreate(handle_meeting_screen, "handle_meeting_screen", 4096, &joystick_data, 5, NULL);
